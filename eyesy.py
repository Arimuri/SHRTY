...
class Eyesy:
    ...
    def __init__(self):
        ...
        self.mode_b = ''
        self.mode_b_root = ''
        self.run_setup_b = False
        self.dual_blend = 0  # 0=add, 1=multiply, 2=crossfade
        self.dual_mix = 0.5  # 0.0=all A, 1.0=all B
        self.DUAL_BLEND_NAMES = ["Add", "Multiply", "Crossfade"]
        ...
        self.MIDI_LEARN_PARAMS = [
            ...
            ("knob4_cc", "Knob 4"),
            ("knob5_cc", "Knob 5"),
            ("auto_clear_cc", "Auto Clear"),
            ("fg_palette_cc", "FG Palette"),
            ("bg_palette_cc", "BG Palette"),
            ("mode_cc", "Mode"),
        ]
    ...
    # SHRTY: dual mode methods
    def set_mode_b_by_index(self, index):
        self.mode_b_index = index
        self.mode_b = self.mode_names[self.mode_b_index]
        self.mode_b_root = self.MODES_PATH + self.mode_b + "/"
        print("setting mode B: " + self.mode_b_root)

    def next_mode_b(self):
        self.mode_b_index = (self.mode_b_index + 1) % len(self.mode_names)
        self.set_mode_b_by_index(self.mode_b_index)
        self.run_setup_b = True

    def prev_mode_b(self):
        self.mode_b_index = (self.mode_b_index - 1) % len(self.mode_names)
        self.set_mode_b_by_index(self.mode_b_index)
        self.run_setup_b = True

    def next_dual_blend(self):
        self.dual_blend = (self.dual_blend + 1) % len(self.DUAL_BLEND_NAMES)

    def toggle_dual_mode(self):
        self.dual_mode = not self.dual_mode
        if self.dual_mode and len(self.mode_names) > 0:
            if self.mode_b_index >= len(self.mode_names):
                self.mode_b_index = 0
            self.set_mode_b_by_index(self.mode_b_index)
            self.run_setup_b = True

    # SHRTY: MIDI learn methods
    ...
    def midi_learn_next_param(self):
        if self.midi_learn_active:
            self.midi_learn_param_index = (self.midi_learn_param_index + 1) % len(self.MIDI_LEARN_PARAMS)

    def midi_learn_prev_param(self):
        if self.midi_learn_active:
            self.midi_learn_param_index = (self.midi_learn_param_index - 1) % len(self.MIDI_LEARN_PARAMS)
