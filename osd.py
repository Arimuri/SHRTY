...
# SHRTY: Dual mode indicator
def render_dual_mode(screen, eyesy):
    font = eyesy.font
    blend_name = eyesy.DUAL_BLEND_NAMES[eyesy.dual_blend]
    mix_pct = int(eyesy.dual_mix * 100)
    mode_a = eyesy.mode_names[eyesy.mode_index] if eyesy.mode_index < len(eyesy.mode_names) else "?"
    mode_b = eyesy.mode_b if eyesy.mode_b else "?"

    # Top bar
    overlay = pygame.Surface((screen.get_width(), 28))
    overlay.fill((0, 60, 0))
    overlay.set_alpha(180)
    screen.blit(overlay, (0, 0))

    txt = f"DUAL  A: {mode_a}  |  B: {mode_b}  |  {blend_name}  |  Mix: {mix_pct}%"
...
# SHRTY: MIDI Learn overlay
def render_midi_learn(screen, eyesy):
    font = eyesy.font
    config_key, name = eyesy.MIDI_LEARN_PARAMS[eyesy.midi_learn_param_index]
    current_cc = eyesy.config.get(config_key, -1)
    cc_str = str(current_cc) if current_cc >= 0 else "none"

    # Bottom bar
    overlay = pygame.Surface((screen.get_width(), 36))
    overlay.fill((0, 0, 120))
    overlay.set_alpha(210)
    screen.blit(overlay, (0, screen.get_height() - 36))

    txt = f"MIDI LEARN: [{name}] (CC {cc_str})  --  move a knob to assign  |  LEFT/RIGHT: param  |  M/ESC: exit"
    text = font.render(txt, True, (255, 255, 0))
    rect = text.get_rect()
    rect.centerx = screen.get_width() // 2
    rect.centery = screen.get_height() - 18
    screen.blit(text, rect)
